...
<script>
import{mapGetters as e,mapMutations as s}from"vuex";export default{name:"Serie",data:()=>({fields:["Where_to_watch"],items:[],serie:[],review:"",comment:"",show:!1,hide:!1,userLogged:"",user:!1,notuser:!1}),created(){this.seri()},computed:{...e(["getShow","getCommentsSeries","getLoggedUser"])},methods:{...s(["SET_COMMENTS_SERIES","SET_FAVORITES_SERIE","SET_WATCHED_SERIE",]),seri(){this.serie=this.getShow(this.$route.params.serieId),this.items=this.serie.whereWatch,console.log(this.$route.params.serieId),console.log(this.serie),this.userLogged=this.getLoggedUser,this.show=!0},onReview(){if(this.userLogged){this.user=!0,this.notuser=!1;let e=new Date().toISOString().substr(0,10);this.SET_COMMENTS_SERIES({idSerie:this.$route.params.serieId,user:this.userLogged.username,photo:this.userLogged.photo,date:e,review:this.review}),console.log(this.comment=this.getCommentsSeries),this.show=!0,this.hide=!1}else this.notuser=!0,this.user=!1;this.sortUsers()},sortUsers(){this.comment.sort(function(e,s){return e.user.length>s.user.length?-1:e.user.length<s.user.length?1:0})},hideComment(){this.show=!1,this.hide=!0},showComment(){this.show=!0,this.hide=!1},addFavorites(e){this.SET_FAVORITES_SERIE(e)},addWatched(e){this.SET_WATCHED_SERIE(e)}}};
</script>
<style>
