...
<script>
import{mapGetters as e,mapMutations as t}from"vuex";export default{name:"Film",data:()=>({film:[],fields:["Where_to_watch"],items:[],review:"",comment:"",show:!1,hide:!1,userLogged:"",user:!1,notuser:!1}),created(){this.filme()},computed:{...e(["getFilm","getComments","getLoggedUser","isId"])},methods:{...t(["SET_COMMENTS","SET_FAVORITES_FILM","SET_WATCHED_FILM"]),filme(){this.film=this.getFilm(this.$route.params.filmeId),this.items=this.film.whereWatch,console.log(this.$route.params.filmeId),console.log(this.film),console.log(this.comment=this.getComments),this.userLogged=this.getLoggedUser,this.show=!0},onReview(){if(this.userLogged){this.user=!0,this.notuser=!1;let e=new Date().toISOString().substr(0,10);this.SET_COMMENTS({idFilme:this.$route.params.filmeId,user:this.userLogged.username,photo:this.userLogged.photo,date:e,review:this.review}),console.log(this.comment=this.getComments),this.show=!0,this.hide=!1}else this.notuser=!0,this.user=!1;this.sortUsers()},sortUsers(){this.comment.sort(function(e,t){return e.user.length>t.user.length?-1:e.user.length<t.user.length?1:0})},hideComment(){this.show=!1,this.hide=!0},showComment(){this.show=!0,this.hide=!1},addFavorites(e){this.SET_FAVORITES_FILM(e)},addWatched(e){this.SET_WATCHED_FILM(e)}}};
</script>
<style>
